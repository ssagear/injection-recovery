#!/share/pkg/python/2.7.14/install/bin/python
...
def data_K2(fitspath):
    ...
    if os.path.isfile(fitspath):
        ...
        targetname = fitspath[75:]
        targetname = targetname[:-23]

        campaign_no = fitspath[75:]
...
def plot(recovered_period, recovered_rprs, total_per, total_rprs, targetname):
    ...
    ybins_csv = [x / 20 for x in range(21)]
    ...
    counts1, _, _ = np.histogram2d(recovered_period, recovered_rprs, bins=(per_segments, ybins_csv))
    counts_tot1, _, _ = np.histogram2d(total_per, total_rprs, bins=(per_segments, ybins_csv))

    for i in range(len(counts1.T)):
        for j in range(len(counts1.T[i])):
            counts1.T[i][j] = counts1.T[i][j]/counts_tot1.T[i][j]
            if np.isnan(counts1.T[i][j]):
                counts1.T[i][j] = 0
    counts1T = np.flip(counts1.T, axis=0)
