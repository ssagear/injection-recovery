...
#opens list of file names, creates list of file names
    with open('/Users/sheilasagear/Dropbox/K2/surfacetest/epicids_txt/229227256.txt') as f:
        content = f.readlines()
...
#and stores (x,y) in list 'data'
    ...
    while (tempindex < filecount):
        file = '/Users/sheilasagear/Dropbox/K2/surfacetest/lcs_txt_files/' + str(content[tempindex])
        file = file.strip('\n')
        file = file.strip('\r')
...
np.savetxt('/Users/sheilasagear/Dropbox/K2/heatmap_inclination_csv/' + targetname + 'test.csv', block_percent, fmt='%f', delimiter=',')
...
plt.title('Fraction of Transits Recovered (' + str(targetname) + ')')
plt.clim(0,1)
plt.colorbar()
