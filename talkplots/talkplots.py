...
#uncomment if extra
    ...
    x = open('/Users/sheilasagear/Dropbox/217976219.txt', 'a')
    for i in range(len(mergedfluxDetrend)):
        x.write('\n' + str(time[i]) + ', ' + str(mergedfluxDetrend[i]))
    x.close()
...
################################################################
    ...
    print(PyAstronomy.pyasl.transitDuration(.0337, .17, .1, 90, 8))
    
    f = fits.open('/Users/sheilasagear/Dropbox/K2/hlsp_k2sff_k2_lightcurve_217976219-c07_kepler_v1_llc.fits')

    QCDPP6 = f[1].header['QCDPP6']

    print(QCDPP6)

    CDPPeff = ((6/1)**0.5)*QCDPP6

    print('CDPPeff = ' + str(CDPPeff))


    

    #days -> hours
    SNR = ((rprs**2)*1000000)*np.sqrt(80/period)*(1/CDPPeff)
    print('SNR = ' + str(SNR))

    #sigmaSNR = ((rprs**2)*10000)/QCDPP6


    #print('SNR = ' + str(sigmaSNR))
...
###################################################################################
    ...
    #pylab.cla()
    #plt.scatter(foldTimes, mergedfluxDetrend, s=3)
    #plt.show()
    ...
    roundtper=[]
    for i in t:
        roundtper.append(round(i/period))
    
    phase = t/period-roundtper

    plt.scatter(phase, mergedfluxDetrend, s=2, color='k')
    plt.plot(phase, fitT.transitmodel, color='r')
    plt.xlabel('Phase')
    plt.ylabel('Normalized Flux')
    plt.ylim(-0.1,0.1)
    plt.title('EPIC 217976219 | 8.0 Day Period | 1.7 $R_e$')
    plt.tight_layout()
    show()
