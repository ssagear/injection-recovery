...
###################################################################################
        ...
        if abs(bestFperiod-period) < .10*period and abs(bestFrprs-rprs) < .10*rprs:
            print('YES')
            recovered = True
            recovered_q.append(results[4])
            recovered_imp.append(impact)
...
ax.scatter(recovered_imp, recovered_q, c='k', s=8)
ax.set_xlabel('Impact Parameter')
ax.set_ylabel('Fractional Transit Duration')
ax.set_xlim(0.0, 1.0)
ax.set_ylim(0.0, 1.0)
show()
