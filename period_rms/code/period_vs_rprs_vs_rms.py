...
ax.set_title('Period vs RMS (red=recovered)')
ax.hold(True)
"""
...
ax.set_xlabel('Period (days)', fontsize=14)
ax.set_ylabel('rprs', fontsize=14)
ax.set_zlabel('RMS', fontsize=14)
ax.hold(True)
...
###################################################################################
        ...
        if abs(bestFperiod-period) < .03*period and abs(bestFrprs-rprs) < .03*rprs:
            ...
            ax.scatter(period, rprs, RMS, c='r', s=8, depthshade=True)
        else:
            """
            recovered = False
            recoveredarr.append(False)
            ax.scatter(period, rprs, RMS, c='b', s=8)
            """
            pass
